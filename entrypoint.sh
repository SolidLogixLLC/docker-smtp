...
echo -n "" > /opt/app-root/exim4.conf.localmacros

if [ "$MAILNAME" ]; then
	echo "MAIN_HARDCODE_PRIMARY_HOSTNAME = $MAILNAME" > /opt/app-root/exim4.conf.localmacros
	echo $MAILNAME > /etc/mailname
fi

if [ "$KEY_PATH" -a "$CERTIFICATE_PATH" ]; then
	if [ "$MAILNAME" ]; then
	  echo "MAIN_TLS_ENABLE = yes" >>  /opt/app-root/exim4.conf.localmacros
	else
	  echo "MAIN_TLS_ENABLE = yes" >>  /opt/app-root/exim4.conf.localmacros
	fi
...
if [ "$DISABLE_IPV6" ]; then
        echo 'disable_ipv6=true' >> /opt/app-root/exim4.conf.localmacros
fi
...
if [ -f /etc/exim4/_docker_additional_macros ]; then
	cat /etc/exim4/_docker_additional_macros >> /opt/app-root/exim4.conf.localmacros
fi
